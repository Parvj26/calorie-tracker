...
import { Save, RotateCcw, User, Target, Footprints, Scale, Copy, Check } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({
  ...
  return (
    <div className="settings">
      ...
      {/* Coach Code Section - Only for coaches */}
      {isCoach && (
        <div className="card settings-card coach-code-settings-card">
          <h3><User size={20} /> Your Coach Code</h3>
          <p className="form-help">
            Share this code with your clients so they can connect with you.
          </p>
          <div className="coach-code-settings-display">
            <span className="coach-code-value">{coachCode || 'Loading...'}</span>
            <button
              className="copy-code-btn"
              onClick={copyCoachCode}
              disabled={!coachCode}
            >
              {copiedCoachCode ? <Check size={18} /> : <Copy size={18} />}
              {copiedCoachCode ? 'Copied!' : 'Copy Code'}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="card settings-card profile-card">
        ...
        {/* These fields are only relevant for regular users, not coaches */}
        {!isCoach && (
          <>
            <div className="form-row">
              <div className="form-group">
      ...
      {/* Daily Nutrition Goals - only for regular users */}
      {!isCoach && (
      ...
      {/* Calorie Target - only for regular users */}
      {!isCoach && (
      ...
      {/* AI Provider - only for regular users */}
      {!isCoach && (
      ...
      {/* Settings actions - only for regular users */}
      {!isCoach && (
      ...
      {/* Data Management - only for regular users */}
      {!isCoach && <div className="card data-card">

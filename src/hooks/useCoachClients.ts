  ...
  const loadClients = useCallback(async () => {
    ...
    try {
      ...
      for (const rel of accepted) {
        ...
        // Load today's daily log (for future calorie calculation)
        const today = new Date().toISOString().split('T')[0];
        const { data: _todayLog } = await supabase
          .from('daily_logs')
          .select('*')
          .eq('user_id', rel.client_id)
          .eq('date', today)
          .single();
        void _todayLog; // Reserved for future calorie calculation
        ...
        clientSummaries.push({
          ...
          latestWeight,
          weightChange7Days,
          caloriesToday: 0, // Would need meal calculation

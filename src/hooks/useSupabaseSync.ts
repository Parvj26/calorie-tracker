...
export function useSupabaseSync() {
    ...
    try {
      ...
      if (settingsRes.data) {
        ...
        settings = {
          ...
          tefMultiplier: settingsRes.data.tef_multiplier || 1.10,
          startWeight: settingsRes.data.start_weight,
          goalWeight: settingsRes.data.goal_weight,
          startDate: settingsRes.data.start_date,
          targetDate: settingsRes.data.target_date,
          weightUnit: settingsRes.data.weight_unit || 'kg',
  ...
  const saveSettings = useCallback(async (settings: UserSettings) => {
    ...
    await supabase.from('user_settings').upsert({
      ...
      tef_multiplier: settings.tefMultiplier || 1.10,
